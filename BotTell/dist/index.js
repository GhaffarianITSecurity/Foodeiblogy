...
async function searchRecipesByIngredients(ingredients) {
    ...
    `;
        ...
        const postsWithIngredients = await Promise.all(posts.map(async (post) => {
            const [ingredientRows] = await pool.query('SELECT name FROM ingredients WHERE post_id = ?', [post.id]);
            return {
                id: post.id,
                title: post.title,
...
async function getAllRecipes(page = 1, limit = 5) {
    try {
...
async function sendRecipeList(chatId, page = 1) {
...
async function sendRecipeDetails(chatId, recipeId) {
    try {
        // Get recipe details
        const [rows] = await pool.query('SELECT id, title, content, image FROM posts WHERE id = ?', [recipeId]);
        if (rows.length === 0) {
        ...
        const recipe = rows[0];
        // Get ingredients
        const [ingredientRows] = await pool.query('SELECT name, amount, unit FROM ingredients WHERE post_id = ? ORDER BY `order`', [recipeId]);
...
bot.onText(/\/start/, (msg) => {
    ...
    bot.sendMessage(chatId, 'سلام! به ربات جستجوی دستور پخت غذا خوش آمدید. 👨‍🍳\n\n' +
...
bot.on('message', async (msg) => {
    ...
    if (recipes.length === 0) {
        bot.sendMessage(chatId, 'متأسفانه دستور پختی با مواد اولیه شما پیدا نشد. لطفاً مواد دیگری را امتحان کنید.');
        return;
    }
    // Send the results
    bot.sendMessage(chatId, `${recipes.length} دستور پخت با مواد اولیه شما پیدا شد:`);
    // Send each recipe
    ...
    if (recipes.length > 5) {
        bot.sendMessage(chatId, `و ${recipes.length - 5} دستور پخت دیگر. برای دیدن همه نتایج به وبسایت مراجعه کنید.`);
    }
});
// Error handling
